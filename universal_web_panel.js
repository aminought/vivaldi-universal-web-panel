(function universal_web_panel() {
    ...
    const PANEL_CHANGE_OBSERVER = new MutationObserver((records) => {
        records.forEach((record) => handlePanelChange(record));
    });
    ...
    class UWP {
        ...
        constructor(panel) {
            ...
            this.webviewChangeObserver = new MutationObserver((records) => {
                records.forEach((record) => this.#handleWebviewChange(record));
            });
            this.webviewChangeObserver.observe(this.#webview, {
                attributes: true,
                attributeFilter: ['src'],
                childList: false,
                subtree: false,
        ...
        #createInputToolbar() {
            ...
            inputToolbar.className = 'panel-universal-input toolbar-default full-width';
        ...
        get #buttonImg() {
            const panelsButtons = document.querySelectorAll('.button-toolbar-webpanel');
    ...
    function getVisiblePanel() {
        return document.querySelector('.panel.webpanel.visible');
    }

    function checkPanel(panel) {
        const panels = getPanels();
        return getPanel(panels) === panel;
    }

    function registerUWP(panel) {
        if (checkPanel(panel) && !window.uwp) {
            console.log('new uwp');
            window.uwp = new UWP(panel);
        } else if (window.uwp) {
            window.uwp.focusInput();
        }
    }

    function beginPanelsObservation(panels) {
        PANEL_CHANGE_OBSERVER.observe(panels, {
            attributes: true,
