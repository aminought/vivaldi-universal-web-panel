(function universal_web_panel() {
    ...
    function showHtml(html, panel, webview) {
        ...
        const title = panel.querySelector('.webpanel-title').querySelector('span');
    ...
    function createUniversalInputToolbar(panel) {
        ...
        universalInputToolbar.className = "panel-universal-input toolbar-default full-width";
    ...
    const WEBVIEW_CHANGE_OBSERVER = new MutationObserver(records => {
        records.forEach(record => {
            ...
            if (src === 'about:blank') {
                src = null;
                const titleSpan = record.target.parentElement.parentElement.querySelector('.webpanel-title').querySelector('span');
